...
function [Ke,Fe,Re,Ve] = impedance(k)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2D Convected Helmholtz Equation
% Notation: exp(-i \omega t)
% Boundary condition: impedance
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Boundary condition values (impedance Z)
Z1 = BOUNDARY_DATA(1,k);
Z2 = BOUNDARY_DATA(2,k);
...
for n=1:N_GAUSS_POINT
    ...
    % Interpolate the impedance
    Z = L*[Z1;Z2];
    % Add contribution to the element matrix
    Ke = Ke + GAUSS_WEIGHT(n)*( -1i*omega*RHO*C0^2/Z*(L'*L) )*Js;
end
