...
Barycenter = [mean(NODE(1,ELEMENT(1:3,k))),mean(NODE(2,ELEMENT(1:3,k))),mean(NODE(3,ELEMENT(1:3,k)))];
% %
% figure(1);
% hold on;
% quiver3(Barycenter(1),Barycenter(2),Barycenter(3),no(1),no(2),no(3));
% axis equal;
...
for n=1:N_GAUSS_POINT
    ...
    if norm(X_tetra-X_tria)>100*eps
        ...
        %plot3(X_tria(1),X_tria(2),X_tria(3),'o')
    end
    ...
    Ke(1:4,1:4) = Ke(1:4,1:4) + GAUSS_WEIGHT(n)*( rho*M*N.'*(   dphi*GRAD - dphi_n*Gn  )/c    )*Js;
    Fe(1:4) = Fe(1:4) + GAUSS_WEIGHT(n)*( -rho*N'*M*c + rho*N.'*M*dphi_n*(M-dphi_n/c) )*Js;
end
